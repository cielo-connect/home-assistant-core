...
def async_handle_api_call(
    ...
    async def wrap_api_call(*args: Any, **kwargs: Any) -> dict[str, Any] | None:
        ...
        # 1. Map API data to Home Assistant entity attributes (device_data model)
        ...
        # 2. Update the internal model/device data immediately
        ...
        # Store the successful action data for the entity
        entity.last_action = data
        entity.last_action_timestamp = int(time())

        # 3. Notify HA to update the UI
        entity.async_write_ha_state()

        # 4. Notify the coordinator to mask the next refresh
...
class CieloBaseEntity(CoordinatorEntity[CieloDataUpdateCoordinator]):
    ...
    def client(self) -> Any:
        ...
        # Ensure the client is bound to the correct device data before use
    ...
    def device_data(self) -> CieloDevice | None:
        """Return the device data from the coordinator, applying action masking and staleness check."""
        current_time = int(time())

        # 1. Staleness Check (Fast return of cached data)
        if (
