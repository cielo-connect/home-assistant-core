...
class CieloConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_reauth(
        ...
        # Store the entry ID in context for use in the next step
    ...
    async def async_step_reauth_confirm(
        ...
        # First time here (user_input is None): show the form
        ...
        # Use the *new* key entered by the user
        api_key = user_input[CONF_API_KEY].strip()
        ...
        try:
            ...
            # Success! Update the entry data and unique ID
            new_data = {
            ...
            # Update unique ID (based on new token)
            self.hass.config_entries.async_update_entry(
                self._reauth_entry, unique_id=token
            )

            # Update the entry, reload the integration, and finish the flow
        ...
        # If we got here, there was an error: show the form again with errors
        return self.async_show_form(
            step_id="reauth_confirm",
            data_schema=DATA_SCHEMA,
            errors=errors,
